...
●説明
 「reces」は、統合アーカイバライブラリ(*.dll)やSusie Plug-in(*.spi)、Total Commander Plugin(*.wcx)を使用し、
...
●インストール
 1.「reces000r23.zip」を適当なディレクトリに解凍して下さい。
 ...
 ・Total Commander Pluginのデフォルトの読込先はreces.exeがあるディレクトリです。
   別のディレクトリを指定する場合は「/Dw」オプションを使用して下さい。
...
●使用方法
 ...
 /m<r|R|c|C|e|E|l|L|s|S|v>
   ・動作モードを指定します。省略された場合、/mrzipとして処理します。
        ...
        /mt[library]
          ...
          ・Total Commander Pluginは対応しています。


        /ms[library][:prefix]
          ・各統合アーカイバライブラリを直接操作します。[library]が省略された場合、7-zip32として処理します。
          ・詳細については[直接操作]をご覧下さい。
 ...
 /eb[level|x]
   ...
   ・「/ebx」で共通するパス全てを除外します。
   ・統合アーカイバライブラリの場合、解凍後にパスの除外を行います。
 ...
 /D<b|s|w><directory>
   ・特殊なディレクトリを指定します。
        ...
        /Dw<directory>
          ...
          ・指定しない場合、reces.exeのあるディレクトリから読み込みます。
          ・Total Commander Pluginを読み込みたくない場合、「/Dw:」など無効なディレクトリ名を指定して下さい。
...
●開発環境
 ...
 compiler:Microsoft Visual C++ 2010 Express/gcc version 4.7.1 (tdm-1)
 debugger:Microsoft Visual C++ 2010 Express/gdb 7.3 with Code::Blocks 10.05
 eeditor:xyzzy version 0.2.2.235/ResEdit 1.6.3 Unicode build.
