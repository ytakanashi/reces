...
●特徴
 ...
 ・プログレスバーを自前で表示し、無駄なウインドウを表示させない。 *2
 ...
 ・レジストリは一切使用しない。 *4
   ...
   *2 一部Dllではプログレスバーの動作に必要な情報が取得できないため、表示されません。
   *3 勿論タイムスタンプが保持されている場合に限ります。
      保持されていない場合、配下のディレクトリやファイルから取得します。(r17より)
...
●インストール
 1.「reces000r20.zip」を適当なディレクトリに解凍して下さい。
 ...
 ・zip書庫解凍はLMZIP32.dll -> 7-zip32.dll -> XacRett.dllの順で確認を行います。
...
●使用例
 ...
 例8.PPMd方式で7z書庫ファイルを作成。
     reces /mc7z /P-m0=ppmd *.txt
 例9.無圧縮zipアルバム(*.zip.mp3)を作成。
...
●注意
 ...
 ・recesでは対象ファイルを「ワイルドカードを用いて」再帰的に検索することはできません。
   ...
   例.カレントディレクトリ以下の全てのファイルを、新規ディレクトリに解凍する。
     ...
     「reces /me /c12 /ne .」
 ・\\?\拡張パスはライブラリ側が対応していない為取り扱いません。
 ・MAX_PATHチェックは入力ファイルに対してのみ行います。他オプションや操作はMAX_PATHを超えるパスに就いて考慮されていません。
