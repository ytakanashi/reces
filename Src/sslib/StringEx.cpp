...
long long filesize2longlong(const TCHAR* size_str){
	...
	_stscanf_s(size_str,_T("%Lf"),&file_size);
#else
	long double file_size=_tcstold(size_str,NULL);
#endif

	//'k'or'kb'
	if(str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-1],'k')||
	   str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-2],'k')){
		result=static_cast<long long>(file_size*1024);
	}else if(str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-1],'m')||
			 str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-2],'m')){
		result=static_cast<long long>(file_size*(1024<<10));
	}else if(str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-1],'g')||
			 str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-2],'g')){
		result=static_cast<long long>(file_size*(1024<<20));
	}else if(str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-1],'t')||
			 str::isEqualCharacterIgnoreCase(size_str[lstrlen(size_str)-2],'t')){
		result=static_cast<long long>(file_size*(1024<<30));
	}else{
		result=static_cast<long long>(file_size);
	}
